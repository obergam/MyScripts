...
class QxdmLogger():
    ...
    def get_cfg(self, pw):
        get_qxdm_cfg = 'curl -s -u admin:{} --connect-timeout 5 -X GET http://192.168.0.1/api/config/system/qxdmproxy'.format(
            pw)
        ...
        try:
            ...
            self.mode = "lan"
            print(
                "Most likely talking to an older router that doesn't have as much of the config under /config/system/qxdmproxy, exception {}".format(
                    e))
            return None, 1
    ...
    while socket_closed == False:
        ...
        if qxdm_logger.filter != "":
            ...
            if qxdm_logger.legacy == True:
                ...
                print(
                    'If not LP4 modem you can specify the filter file on the command line: --filter_file=file.sqf, or specify it in /config/system/qxdmproxy/filter')
        ...
        filename = now.strftime(
            '{}{}_%Y_%m_%d_%H_%M_%S.qmdl'.format(qxdm_logger.log_base, str(qxdm_logger.set_cnt).zfill(3)))
